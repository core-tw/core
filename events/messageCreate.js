        ...
        try {
            ...
            let cmd =
                ...
                client.commands.find(cmd => {
                    if (cmd.superior) {
                        return (cmd.superior == commandName && cmd.name.includes(args[0]));
                    } else {
            ...
            for (let p in cmd.requireBotPermissions) {
                if (!msg.guild.me.permissions.has(cmd.requireBotPermissions[p])) {
                    errorEmbed(
                        ...
                        author,
                        `權限錯誤`,
                        `需求 :**${cmd.requireBotPermissions[p]}**`
                    );
                    return;
                }
            }
            ...
            if (cmd.cooldown) {
                ...
                if (timestamps.has(author.id)) {
                    ...
                    if (now < expirationTime) {
                        ...
                        msg.reply({
                            content: `指令冷卻中，請於 **${timeLeft.toFixed(
                                1
                            )}** 秒後再次嘗試`,
